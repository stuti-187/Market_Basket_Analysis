...
<?php
    ...
    class Admin {
        ...
        function getunpaidorders(){
                $q = "SELECT * FROM `order` where status='unconfirmed' order by dateOrdered desc";
                $result = mysql_query($q);
            
                return $result;
        }
        function getdeliveredorders(){
                $q = "SELECT * FROM `order` where status='delivered' order by dateDelivered desc";
                $result = mysql_query($q);
            
                return $result;
        }
        function getpaidorders(){
                $q = "SELECT * FROM `order` where status='confirmed' order by dateDelivered desc";
                $result = mysql_query($q);
        ...
        function deliver(){
            ...
            $id = $_GET['id'];
            $q = "UPDATE `order` set dateDelivered='$date', status='delivered' where id=$id";
            mysql_query($q);
        ...
        function paid(){
            $id = $_GET['id'];
            $date = date('m/d/y h:i:s A');
            $q = "UPDATE `order` set dateDelivered='$date', status='confirmed' where id=$id";
            mysql_query($q);
        ...
        function addcategory($cat){
            $q = "INSERT INTO category values('','$cat')";
            mysql_query($q);
            return true;
        }
        ...
        function updatecategory($cat,$id){
            $q = "update category set title='$cat' where id=$id";   
            mysql_query($q);
            return true;
        }
					...
					<div class="col-sm-6">
						<div class="contactinfo">
							<ul class="nav nav-pills">
								...
								<li><a href="#"><i class="fa fa-envelope"></i> rkproject24.com</a></li>
							</ul>
						</div>
					</div>
					<div class="col-sm-6">
